module datetime_module
  ...
  ! Константы - коэффициенты преобразования одних единиц времени в другие
  ...
  integer, parameter :: MAXSTRLEN = 99                          ! максимальная длина строки для strftime
  ...
  type,bind(c) :: tm_struct
    ! Производные типы для совместимости со структурой tm в C и C++.
    ! Позволяют вызывать strftime и strptime используя iso_c_binding.
    ! см: http://www.cplusplus.com/reference/ctime/tm.
  ...
  interface
    ...
    function c_strftime(str, slen, format, tm) bind(c, name='strftime') result(rc)
      ! Возвращает форматированную строку со временем, в соответствии с переданными через аргументы временем и форматом.
      ! См: https://www.cplusplus.com/reference/ctime/strftime.
      import :: c_char, c_int, tm_struct
      character(kind=c_char), intent(out) :: str(*)             ! строка с результатом
      integer(c_int), value, intent(in) :: slen                 ! длина строки
      character(kind=c_char), intent(in) :: format(*)           ! формат времени
      type(tm_struct), intent(in) :: tm                         ! tm_struct instance
      integer(c_int) :: rc                                      ! код возврата
    end function c_strftime

    function c_strptime(str,format,tm) bind(c,name='strptime') result(rc)
      ! Интерфейс к POSIX strptime.
      ! Возвращает tm_struct со временем, в соответствии с переданными через аргументы строкой str и её форматом.
      ! См: http://man7.org/linux/man-pages/man3/strptime.3.html.
      import :: c_char, c_int, tm_struct
      character(kind=c_char), intent(in) :: str(*)              ! входная строка
      character(kind=c_char), intent(in) :: format(*)           ! формат строки
      type(tm_struct), intent(out) :: tm                        ! результат tm_struct
      integer(c_int) :: rc                                      ! код возврата
...
contains
  ...
  pure elemental subroutine reset(self)
    ! сбрасывает часы в исходное состояние
  ...
  pure elemental subroutine tick(self)
    ! приращивает currentTime в clock на величину tickInterval
    class(clock), intent(in out) :: self
  ...
  pure elemental type(datetime) function datetime_constructor( &
    ...
    ! конструктор для класса `datetime`
    integer, intent(in), optional :: year, month, day, hour, minute, second, millisecond
    real(real64), intent(in), optional :: tz                    ! timezone - сдвиг зоны в часах
